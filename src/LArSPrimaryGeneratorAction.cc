...
LArSPrimaryGeneratorAction::LArSPrimaryGeneratorAction(G4String fName)
{
   m_pMessenger       = new LArSPrimaryGeneratorMessenger(this);

  G4cout <<"LArSPrimaryGeneratorAction::LArSPrimaryGeneratorAction() Use LArSParticleSource "<<G4endl;
  m_pParticleSource = new LArSParticleSource();
...
LArSPrimaryGeneratorAction::~LArSPrimaryGeneratorAction()
{
...
void 
LArSPrimaryGeneratorAction::GeneratePrimariesStandard(G4Event *pEvent)
{
...
void 
LArSPrimaryGeneratorAction::FillHistograms(){
...
void 
//LArSPrimaryGeneratorAction::GeneratePrimariesAccelerated(G4Event *pEvent)
//{
LArSPrimaryGeneratorAction::GeneratePrimariesAccelerated(void){ //modified by Patricia 
  ...
  while(!acceptEvent){
    ...
    if(nintersect == 2) {
         ...
         VarianceReductionMode == KILL_FT_PATH)
      {
        ...
        G4Exception("LArSPrimaryGeneratorAction::GeneratePrimariesAccelerated()","GeneratePrimary000",
                    FatalException,msg);
      }
    } 
    ...
    if(nreject>ntry_max){
      ...
      G4Exception("LArSPrimaryGeneratorAction::GeneratePrimariesAccelerated()","GeneratePrimary001",FatalException,msg);
    }
  }
  ...
  if(m_hParticleTypeOfPrimary != "gamma"){
    ...
    G4Exception("LArSPrimaryGeneratorAction::GeneratePrimariesAccelerated()","GeneratePrimary002", 
                FatalException,msg);
  }
  ...
  FillHistograms();
#else
  G4Exception("LArSPrimaryGeneratorAction::GeneratePrimariesAccelerated()","Accelerate000",FatalException,
              "Accelerated primary generation only supported with _use_GPS_");
#endif
}

void
LArSPrimaryGeneratorAction::GeneratePrimaries(G4Event *pEvent)
{
  //
...
G4int
LArSPrimaryGeneratorAction::IntersectWithTarget(G4ThreeVector x0, G4ThreeVector dir)
{
  ...
  if(D>=0){ // solutions for an intersect with a straight line and cylinder
    ...
    if(step[0]*step[1]<0){
      ...
      G4Exception("LArSPrimaryGeneratorAction::IntersectWithTarget()","Intersect000",FatalException,msg);
    }
  }
...
G4double 
LArSPrimaryGeneratorAction::ComputeGammaMeanFreePath(G4double e, G4String matName)
{
  //
...
G4double
LArSPrimaryGeneratorAction::ComputeForcedTransportWeight(G4ThreeVector x0, G4ThreeVector dir, 
